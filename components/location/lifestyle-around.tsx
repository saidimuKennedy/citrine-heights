...
interface LifestyleAroundProps {
  lifestyle: LocationContent["lifestyle"];
}

export function LifestyleAround({ lifestyle }: LifestyleAroundProps) {
  const [activeIndex, setActiveIndex] = useState(0);
  const activeMoment = lifestyle.moments[activeIndex];

  return (
    <section className="bg-[#111111] px-6 py-24 md:px-16 md:py-32">
      <div className="mx-auto max-w-7xl">
        <MotionReveal y={24} className="mb-12 md:mb-16">
          <EditorialLabel>{lifestyle.label}</EditorialLabel>
        ...
        <div className="mb-10 flex flex-wrap gap-2 md:gap-3">
          {lifestyle.moments.map((moment, idx) => (
            <button
              key={moment.period}
              type="button"
              onClick={() => setActiveIndex(idx)}
              className={`px-5 py-3 text-[10px] font-semibold tracking-[0.28em] transition-all duration-500 md:px-7 md:text-[11px] ${
                idx === activeIndex
